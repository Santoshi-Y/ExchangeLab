...
bool send_all(int socket, std::span<const std::byte> bytes) {
...
bool receive_exact(int socket, std::span<std::byte> output) {
...
bool send_order(
    ...
    const auto body = exchange::protocol::encode_new_order(request);
...
std::optional<ReceivedMessage> receive_message(int socket) {
    std::array<std::byte, exchange::protocol::header_size> header_bytes {};
    if (!receive_exact(socket, header_bytes)) return std::nullopt;

    const auto header = exchange::protocol::decode_header(header_bytes);
...
void print_message(const ReceivedMessage& message) {
    ...
    switch (message.header.type) {
        ...
        case MessageType::OrderRejected: {
            const auto response =
                exchange::protocol::decode_order_response(message.body);
            if (!response.has_value()) {
                std::cout << "Invalid order response\n";
                return;
            }
            std::cout
                << "ORDER RESPONSE id=" << response->order_id
        ...
        case MessageType::BookUpdate: {
            const auto update =
                exchange::protocol::decode_book_update(message.body);
            if (!update.has_value()) {
            ...
            std::cout << "BOOK ";
            if (update->has_bid != 0) {
                std::cout << "bid=" << update->best_bid
                          << " x " << update->bid_quantity << ' ';
            } else {
                std::cout << "bid=none ";
            }
            if (update->has_ask != 0) {
                std::cout << "ask=" << update->best_ask
        ...
        case MessageType::Level3AddOrder: {
            const auto event =
                exchange::protocol::decode_level3_add_order(message.body);
            if (!event.has_value()) {
        ...
        case MessageType::Level3OrderExecuted: {
            const auto event =
                exchange::protocol::decode_level3_order_executed(
                    message.body
                );
            if (!event.has_value()) {
                std::cout << "Invalid Level-3 execution event\n";
                return;
            }
            std::cout
        ...
        case MessageType::Level3OrderDeleted: {
            const auto event =
                exchange::protocol::decode_level3_order_deleted(
                    message.body
                );
            if (!event.has_value()) {
                std::cout << "Invalid Level-3 delete event\n";
                return;
            }
            std::cout << "L3 DELETE id=" << event->order_id << '\n';
            break;
        }
        ...
        default:
            std::cout
                ...
                << static_cast<std::uint16_t>(message.header.type)
                << '\n';
            break;
    }
}
...
int main() {
    ...
    if (!send_order(client_socket, sell_order, 1)) {
        std::cerr << "Failed to send order\n";
        ::close(client_socket);
        return 1;
    }

    // The server sends: OrderAccepted, BookUpdate, then Level3AddOrder.
    for (int index = 0; index < 3; ++index) {
        const auto message = receive_message(client_socket);
        if (!message.has_value()) {
            std::cerr << "Failed to receive server message\n";
            ::close(client_socket);
            return 1;
        }
